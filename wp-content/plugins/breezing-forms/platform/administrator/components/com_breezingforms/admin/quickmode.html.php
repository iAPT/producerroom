...
class QuickModeHtml{
	...
	public static function showApplication($formId = 0, $formName, $formTitle, $formDesc, $formEmailntf, $formEmailadr, $dataObjectString, $elementScripts, $themes){
		...
		$iconBase = '../wp-content/plugins/'.BF_FOLDER.'/platform/administrator/components/com_breezingforms/libraries/jquery/themes/quickmode/i/';
                $iconBaseWp = BF_PLUGINS_URL . '/'.BF_FOLDER.'/platform/administrator/components/com_breezingforms/libraries/jquery/themes/quickmode/i/';
?>              
        ...
        function keepAlive() {	var myAjax = new Request({method: "get", url: "admin.php?page=breezingforms"}).send();} window.addEvent("domready", function(){ keepAlive.periodical(840000); });
                        window.addEvent('domready', function() {
	...
	function BF_QuickModeApp(){
		...
		this.dataObject = <?php echo $dataObjectString ?>;
		...
		*/
		...
		this.recreatedIds = function(startObj){
			...
			return null;
                ...
                this.applyIcons = function(startObj){
                        if( startObj && startObj.attributes && startObj.attributes.id ){
                                startObj.data.icon = '<?php echo $iconBaseWp ;?>' + bf_basename(startObj.data.icon);
				if(startObj.children){
					var child = null;
					for(var i = 0; i < startObj.children.length; i++){
                                                child = appScope.applyIcons(startObj.children[i]);
						if(child){
							return child;
						}
					}
		...
		*/
		...
		this.saveTextProperties = function(mdata, item){
			...
			item.properties = mdata;
                        item.data.icon = '<?php echo $iconBaseWp ;?>' + bf_basename(item.data.icon);
		...
		this.populateFileProperties = function(mdata){
			...
			JQuery('#bfElementFileAdvancedHideLabel').attr('checked', mdata.hideLabel);
                        if(mdata.useUrl && mdata.useUrlDownloadDirectory == ''){
                            mdata.useUrlDownloadDirectory = '<?php echo WP_CONTENT_URL . '/breezingforms/uploads'  ;?>';
                        }
		...
		*/
		...
		JQuery('#bfElementExplorer').tree(
			{
                          path : '<?php echo plugins_url() . '/'.BF_FOLDER. '/platform/administrator/components/com_breezingforms/libraries/jquery/jtree/';?>',
			  ui : {
			    theme_name : "apple",
			    context: [
					{
	...
	var bf_submitbutton = function (pressbutton)
	{
		...
		switch (pressbutton) {
                        ...
                        case 'close':
                            location.href="admin.php?page=breezingforms&act=manageforms";
                            break;
			case 'save':
...
<div style="float:right; margin-top: 10px; margin-right: 15px;">
    ...
    if($formId != 0){
    ?>
    <button onclick="bf_submitbutton('preview')" class="button">
    <?php echo BFText::_('COM_BREEZINGFORMS_TOOLBAR_QUICKMODE_PREVIEW');?>
    </button>
    <?php
    }
    ?>

    <button onclick="bf_submitbutton('close')" class="button">
    <?php echo BFText::_('COM_BREEZINGFORMS_TOOLBAR_QUICKMODE_CLOSE');?>
    </button>
</div>
...
<div style="float:left; margin-top: 10px;  margin-bottom: 30px;">
        <form onsubmit="return false;">
                ...
                <span class="bfFadingMessage" style="display:none; margin-top: 20px;"></span>
        </form>
</div>
...
<div id="bfQuickModeWrapper" class="bfClearfix">
	...
	<div id="menutab" class="flora">
			...
			<div class="m">
	            ...
	            <div id="fragment-1">
		            <div>
		            	<div class="bfFadingMessage" style="display:none; margin-top: 20px;"></div>
		            	<input style="margin-top: 20px;" type="submit" value="<?php echo BFText::_('COM_BREEZINGFORMS_PROPERTIES_SAVE'); ?>" id="bfPropertySaveButtonTop"/>
		            	...
		            	<div class="bfProperties" id="bfPageProperties" style="display:none">
		            		...
		            		<a href="admin.php?page=breezingforms&option=com_breezingforms&tmpl=component&act=quickmode_editor" title="<?php echo BFText::_('COM_BREEZINGFORMS_EDIT_INTRO');?>" class="modal" rel="{handler: 'iframe', size: {x: 820, y: 400}}"><?php echo BFText::_('COM_BREEZINGFORMS_EDIT_INTRO'); ?></a>
		            		</fieldset>
		            	</div>
		            	<!-- PAGE PROPERTIES END -->
		            	...
		            	<div class="bfProperties" id="bfSectionProperties" style="display:none">
		            		...
		            		<fieldset>
		            			...
		            			<a href="admin.php?page=breezingforms&option=com_breezingforms&tmpl=component&act=quickmode_editor" title="<?php echo BFText::_('COM_BREEZINGFORMS_EDIT_DESCRIPTION');?>" class="modal" rel="{handler: 'iframe', size: {x: 820, y: 400}}"><?php echo BFText::_('COM_BREEZINGFORMS_EDIT_DESCRIPTION'); ?></a>
		            		</fieldset>
		            	</div>
		            	...
		            	<div class="bfFadingMessage" style="display:none; margin-bottom: 20px;"></div>
		            	<input type="submit" style="margin-bottom: 20px;" value="<?php echo BFText::_('COM_BREEZINGFORMS_PROPERTIES_SAVE'); ?>" id="bfPropertySaveButton"/>
		            </div>
	            </div>
	            ...
	            <div id="fragment-2">
	            	<div>
	            		<div class="bfFadingMessage" style="display:none; margin-top: 20px;"></div>
			            <input style="margin-top: 20px;" type="submit" value="<?php echo BFText::_('COM_BREEZINGFORMS_PROPERTIES_SAVE'); ?>" id="bfAdvancedSaveButtonTop"/>
			            ...
			            <div class="bfAdvanced" id="bfElementAdvanced" style="display:none">
			            	...
			            	<fieldset>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeFileAdvanced" style="display:none">
			            			...
			            			<input type="checkbox" value="" id="bfElementFileAdvancedFlashUploader" onclick="alert('Flash Uploader is only available in the full version of BreezingForms');"/>
			            			<br/>
			            			<br/>
			            ...
			            <div class="bfFadingMessage" style="display:none;  margin-bottom: 20px;"></div>
			            <input type="submit" style="margin-bottom: 20px;" value="<?php echo BFText::_('COM_BREEZINGFORMS_PROPERTIES_SAVE'); ?>" id="bfAdvancedSaveButton"/>
	            	</div>
	            </div>
            </div>
